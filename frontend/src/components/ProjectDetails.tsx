...
const ProjectDetails: React.FC = () => {
    ...
    const handleCreateMeeting = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const updatedProject = await fetch(`http://localhost:8000/projects/${projectId}`).then(r => r.json());
                setProject(updatedProject);

                // Trigger auto-join
                const meeting = await res.json();
                fetch(`http://localhost:8000/meetings/${meeting.id}/join`, { method: 'POST' }).catch(console.error);
            }
        } catch (err) {
            console.error(err);
    ...
    if (!project) return <div className="p-5 text-center text-danger">Project not found</div>;

    return (
        <div className="container mt-4">
            <Link to="/" className="btn btn-link ps-0 mb-3 text-decoration-none">&larr; Back to Dashboard</Link>
                ...
                <ListGroup variant="flush">
                    {project.meetings.length === 0 ? (
                        ...
                        project.meetings.map(meeting => (
                            <ListGroup.Item key={meeting.id} className="d-flex justify-content-between align-items-center p-3">
                                <div>
                                    <div className="d-flex align-items-center gap-2">
                                        <h5 className="mb-0">Meeting #{meeting.id}</h5>
                                        <Badge bg={meeting.platform === 'zoom' ? 'primary' : 'warning'}>
                                            {meeting.platform}
                                        </Badge>
                                    </div>
                                ...
                                <Link to={`/meeting/${meeting.id}`}>
                                    <Button variant="outline-primary" size="sm">Open Workspace &rarr;</Button>
                                </Link>
                            </ListGroup.Item>
                        ))
                    )}
